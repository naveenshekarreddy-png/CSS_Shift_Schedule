...
st.title("🌙 ☀️ 24/7 Automated Roster Tool")
...
def generate_roster():
    ...
    for i in range(duration):
        ...
        # Condition: Filter out people on leave AND people who worked 6 days straight
        ...
        def pick_for_shift(shift_name, already_selected):
            ...
            for tier, members in staff_data.items():
                ...
                # Eligibility: In tier + available + not picked for other shift today
                eligible = [s for s in members if s in available_today and s not in already_selected]
                
                # Condition 4: Favor shift consistency (5-6 day blocks)
        ...
        # Update trackers for the next day
...
if st.button("🚀 Generate Roster (Excluding Leave)"):
    if len(on_leave) > 10:
        st.warning("⚠️ Warning: Many staff on leave. Roster might be difficult to balance.")
        
    df = generate_roster()
    st.table(df) # Changed to table for better visibility
    
    csv = df.to_csv(index=False).encode('utf-8')
    st.download_button("📥 Download Spreadsheet", csv, "team_roster_final.csv", "text/csv")